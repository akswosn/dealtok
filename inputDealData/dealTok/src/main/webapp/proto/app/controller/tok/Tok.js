...
Ext.define('Rnx.controller.tok.Tok', {
	...
	config : {
		refs : {
			inTokList : 'tokMainTab #inTokList',
			outTokList : 'tokMainTab #outTokList',
		...
		control : {
			...
			inTokList : {
				...
				activate:function(list){
					...
					list.getStore().getProxy()._url = Rnx.app.serverBaseUrl + '/rnxApi/tok/tokGroupList/in/' + Rnx.app.loginSeq;
			...
			outTokList : {
				...
				activate:function(list){
					...
					list.getStore().getProxy()._url = Rnx.app.serverBaseUrl + '/rnxApi/tok/tokGroupList/out/' + Rnx.app.loginSeq;
	...
	showTokDetail:function(list, idx, target, record){
			...
			dealItem.setHtml('<img width="100%" src="resources/images/hotTalkImg/'+ dealImg +'"/>'
			+ '<p>' + dealSummary  + '</p>');
